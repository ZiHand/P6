  ...
  Sauce.findOne({ _id: req.params.id })
    .then((sauce) => {
      if (sauce) {
        ...
        if (!req.file) {
          ...
          reqSauce.imageUrl = `${req.protocol}://${req.get("host")}/images/${
            req.file.filename
          }`;
          reqSauce.usersLiked = sauce.usersLiked;
          reqSauce.usersDisliked = sauce.usersDisliked;
    ...
    .then((sauce) => {
      Sauce.updateOne({ _id: req.params.id }, sauce)
        ...
        .catch((error) =>
          res.status(400).json({ message: "Sauce modification FAILED !" })
        );
    })
  ...
  Sauce.findOne({ _id: req.params.id })
    .then((sauce) => {
      ...
      if (req.body.like > 0) {
        ...
        if (
          !sauce.usersDisliked.find((element) => element === req.body.userId)
        ) {
      ...
      if (shouldUpdate) {
        ...
        Sauce.updateOne({ _id: req.params.id }, sauce)
          .then(() =>
            res.status(200).json({ message: "Like / dislike sauce done !" })
          )
          .catch((error) =>
            res.status(400).json({ message: "Like / dislike sauce FAILED !" })
          );
      } else {
